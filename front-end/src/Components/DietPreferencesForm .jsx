...
const ComponentTwo = ({ onNext, onBack, componentOneData }) => {
    ...
    const [errors, setErrors] = useState({ activityLevel: '' });
    ...
    const handleChange = (e) => {
        ...
        if (name === 'activityLevel') {
            setErrors(prev => (prev.activityLevel ? { ...prev, activityLevel: '' } : prev));
    ...
    const handleSubmit = (e) => {
        ...
        if (!formData.activityLevel) {
            setErrors({ activityLevel: 'Select an activity level to continue' });
            return;
        }
    ...
    return (
        <div className="mx-auto p-6 bg-white rounded-lg shadow-md">
            <h2 className="text-2xl font-bold text-gray-800 mb-6">Lifestyle Preferences (Optional)</h2>
            <p className="text-sm text-gray-600 mb-6">Optional inputs help the optimization engine apply health constraints and tailor recommendations. If skipped, the system uses safe default values.</p>
<form onSubmit={handleSubmit} noValidate>
                <div className="mb-6">
                    ...
                    {errors.activityLevel ? (
                        <div className="text-xs text-red-600 mt-2">{errors.activityLevel}</div>
                    ) : null}
