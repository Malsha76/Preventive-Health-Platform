...
function computeAdherenceScore(activities) {
  ...
  score += Math.min(Number(activities.walkMinutes || 0), 30) * (20 / 30); 
  score += Math.min(Number(activities.sleepHours || 0), 8) * (15 / 8); 
  score += Math.min(Number(activities.waterLiters || 0), 2) * (5 / 2); 
