...
class DataPreprocessor:
    ...
    def preprocess_data(self, apply_scaling=True):
        if self.df is None:
        ...
        # Ensure all required health-related columns exist with sensible fallbacks / aliases
        health_columns = [
            'sugar_g', 'added_sugar_g', 'sodium_mg', 'saturated_fat_g',
            'fiber_g', 'cholesterol_mg', 'potassium_mg', 'omega3_g'
        ]

        # Known alias mapping for datasets that use different headers
        alias_map = {
            'added_sugar_g': [
                'added_sugars_g', 'added_sugar', 'sugars_added_g',
                'sugars_added', 'addedSugars', 'added_sugar_grams'
            ],
            'sugar_g': ['sugars_g', 'sugar', 'total_sugars_g'],
            'sodium_mg': ['sodium', 'sodium_milligrams'],
            'saturated_fat_g': ['saturated_fat', 'sat_fat_g', 'saturatedFat_g'],
            'fiber_g': ['fibre_g', 'fiber'],
            'cholesterol_mg': ['cholesterol', 'cholesterol_milligrams'],
            'potassium_mg': ['potassium', 'potassium_milligrams'],
            'omega3_g': ['omega_3_g', 'omega3', 'omega-3_g']
        }

        for col in health_columns:
            if col not in self.df.columns:
                # Try to find an alias and map it
                source = None
                ...
                if source is not None:
                    self.df[col] = self.df[source]
                    print(f"Info: Using alias column '{source}' for '{col}'")
                else:
                    if col == 'added_sugar_g':
                        # Treat as optional in many datasets; default to 0 without alarming warning
                        print("Info: 'added_sugar_g' not found; defaulting to 0")
                    else:
        ...
        if apply_scaling:
            ...
            print("Skipping MinMax scaling (raw nutrition values kept for GA / evaluation).")
