...
import pickle
from config import DIET_TYPE_MAPPING, DEFAULT_NUTRITION_TARGETS
...
df, _, _, _ = preprocessor.preprocess_data(apply_scaling=False)

# Initialize Genetic Algorithm (nutrients must be same scale as daily calorie targets)
ga = MealPlanGeneticAlgorithm(df)

def calculate_target_nutrition(user_data):
    """Daily target calories only (simplified requirement)."""
    calories = float(user_data.get('adjustedCalories', 2000))
    ...
    try:
        ...
        for day_idx, day_meals in enumerate(best_solution):
            ...
            for meal_type, meal_idx in day_meals.items():
                ...
                serving_size = 100
                current_nutrition['calories'] += float(selected_meal['calories']) * (serving_size / 100)
                current_nutrition['protein'] += float(selected_meal['protein']) * (serving_size / 100)
                current_nutrition['fat'] += float(selected_meal['fat']) * (serving_size / 100)
                current_nutrition['carbs'] += float(selected_meal['carbs']) * (serving_size / 100)

                # Add meal to plan
                meal_data = {
                    'meal_name': selected_meal['meal_name'],
                    'calories': float(selected_meal['calories']) * (serving_size / 100),
                    'protein': float(selected_meal['protein']) * (serving_size / 100),
                    'fat': float(selected_meal['fat']) * (serving_size / 100),
                    'carbs': float(selected_meal['carbs']) * (serving_size / 100),
